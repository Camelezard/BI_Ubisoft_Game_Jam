...
public class DialogManager : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        //eventInstance = RuntimeManager.CreateInstance(SoundManager.Instance.lOsirisDialogue);
    }
    ...
    private IEnumerator DialogUIAppear()
    {
        ...
        _coroutineCharacterTalk = StartCoroutine(CharacterTalkVisualCoroutine(false));
        _dialogBoxText.text = string.Empty;
    ...
    private IEnumerator CharacterTalkVisualCoroutine(bool pLunchSoonInstance = true)
    {
        ...
        if (pLunchSoonInstance)
        {
            if (lIncreasingName == _dialogNameRight)
            {
                eventInstance = RuntimeManager.CreateInstance(SoundManager.Instance.lSethDialogue);
            }
            else
            {
                eventInstance = RuntimeManager.CreateInstance(SoundManager.Instance.lOsirisDialogue);
